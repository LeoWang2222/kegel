...
npm test（27 项）与 npm run check 均通过。没有连接真实 iPhone，不能把浏览器检查称作真机验证。进一步手感迭代请使用真实设备反馈。

## 5.2 音乐

- music.mjs 合成原创 24 秒双声道「微光」，22050Hz 缓冲区按需生成一次；构建器内联到 app.js，无网络音频资源。
- music 默认 false，musicVolume 默认 35；清空练习记录保留这些偏好。独立于原 sound 提示音。
- AmbientMusic 独立增益：开启淡入 0.8 秒，暂停/退出淡出 0.4 秒；后台或音频中断立即停止所有音源；继续接续位置，退出重置位置。音源结束时断开节点。
- unlockAudio 仅由用户动作调用，await resume 后重新检查当前训练和偏好，防止异步恢复造成退出后播放；系统音频中断同时暂停训练。
